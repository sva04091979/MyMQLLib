...
public:
   ...
   CMQL4MA(uint mPeriod,int mShift,ENUM_MA_METHOD mMethod,ENUM_APPLIED_PRICE mPrice):
      ...
      cPeriod(mPeriod),cShift(mShift),cMethod(mMethod),cPrice(mPrice){cBuf=0;}
...
//-------------------------------------------------------------------------------
void CMQL4MA::ChangeParam(string mSymbol,ENUM_TIMEFRAMES mFrame,uint mPeriod,int mShift,ENUM_MA_METHOD mMethod,ENUM_APPLIED_PRICE mPrice){
   CMQL4IndicatorBase<CMQL4MA>::ChangeParam(mSymbol,mFrame);
   cPeriod=mPeriod;
   cShift=mShift;
   cMethod=mMethod;
   cPrice=mPrice;
